...
const { PLAYLIST_FILENAME, get_playlist_stats } = require('./show-dir/consts');
...
module.exports = function createMiddleware(_dir, _options) {
  ...
  return function middleware(req, res, next) {
    ...
    const isPlaylistFile = filename === PLAYLIST_FILENAME;
      ...
      if (isPlaylistFile) {
        ...
        import('globby')
          .then(({globby}) => {
            ...
            const parent_folder = path.dirname(file);
            console.log({parent_folder})

            globby([
              '**/*.ts',
              '**/*.mp4',
            ], {cwd: parent_folder}).then(videos => {
              console.log({videos})
              
              const contentType = 'application/pls+xml';
              res.setHeader('content-type', contentType);

              res.statusCode = 200;

              const video_parent_folder = path.dirname(path.relative(path.join('/', baseDir), pathname));
              console.log({video_parent_folder});

              videos = videos.map(video => {
                return `File=${publicURL}/${path.join(video_parent_folder, video)}`;
              })
            ...
            console.log({file, pathname, parent_folder});
          })
    ...
    const shouldTryBrotli = !isPlaylistFile && opts.brotli && shouldCompressBrotli(req);
    const shouldTryGzip = !isPlaylistFile && opts.gzip && shouldCompressGzip(req);
